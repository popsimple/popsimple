...
public class TextEditUtils {
	...
	protected static Point2D autoSizeWidget(Widget widget, String html,
			boolean usePreWhiteSpace) {
		...
		int newWidth = (testerWidth + comfortZone) >= minWidth ? testerWidth
				+ comfortZone : minWidth;
	...
	private static void copyTextSizingProps(Style targetStyle, Style widgetStyle) {
		String[] copyProps = new String[]{"fontFamily", "fontSize",
				"fontWeight", "fontStyle", "textTransform", "textDecoration",
				"letterSpacing", "wordSpacing", "lineHeight", "textAlign",
				"verticalAlign", "direction", "padding", "border", "margin",
				"whiteSpace"};
		for (String propName : copyProps) {
			targetStyle
					.setProperty(propName, widgetStyle.getProperty(propName));
		}
	}
	 ...
	 */
	protected static <T extends Widget & HasText> int estimateCharPos(T widget,
			int relativeX) {
