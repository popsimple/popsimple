...
public abstract class ZIndexAllocator {
	...
	private static ArrayList<Element> getElementsToAboveOverlapping(
			Element element) {
		ArrayList<Element> elements = new ArrayList<Element>();

		int currentZIndex = getElementZIndex(element);
		for (Entry<Integer, Element> entry : _zIndexMap.tailMap(currentZIndex)
				.entrySet()) {
			Element nextElement = entry.getValue();
			if (element == nextElement) {
				continue;
			}
			if (false == ElementUtils.isOverlappingElements(element,
					nextElement)) {
	...
	private static ArrayList<Element> getElementsToBelowOverlapping(
			Element element) {
