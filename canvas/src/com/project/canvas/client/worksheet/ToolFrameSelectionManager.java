...
public class ToolFrameSelectionManager 
{
    ...
    public ToolFrameSelectionManager(WorksheetView worksheetView, Widget container, 
            Widget dragPanel, Widget selectionPanel, SimpleEvent<Void> stopOperationEvent)
    {
        this._worksheetView = worksheetView;
        this._container = container;
        this._selectionPanel = selectionPanel;
        this._selectionDragManager = new ElementDragManagerImpl(container, dragPanel, stopOperationEvent);
    }
    ...
    public void startSelectionDrag(MouseDownEvent event) {
        ...
        ElementUtils.setElementPosition(initialPosition, this._selectionPanel.getElement());
        ...
        Handler<Point2D> mouseMoveHandler = new Handler<Point2D>() {
            ...
            public void onFire(Point2D arg) {
                ElementUtils.setElementSize(_selectionPanel.getElement(), arg);

                selectFramesByRectangle(
                        ElementUtils.getElementOffsetRectangle(_selectionPanel.getElement()), 
                        newlySelectedFrames);
            }
        };
        ...
        this._selectionDragManager.startMouseMoveOperation(this._selectionPanel.getElement(), 
    ...
    private void selectFramesByRectangle(Rectangle selectionRectangle, HashSet<CanvasToolFrame> selectedFrameSet)
    {
        for (CanvasToolFrame toolFrame : this._worksheetView.getToolFrames()) {
            if (selectionRectangle.isOverlapping(
                    ElementUtils.getElementOffsetRectangle(toolFrame.getElement()))) {
