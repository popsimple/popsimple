...
GENTICS.Aloha.Format.sizes = [ 6, 8, 10, 12, 14, 16, 18, 20, 24, 32, 48, 72, 96 ];
...
GENTICS.Aloha.Format.initButtons = function() {
	...
	jQuery.each(GENTICS.Aloha.Format.config,
			function(j, button) {
				switch (button) {
				case "b":
				case "i":
				...
				case "sup":
					...
					GENTICS.Aloha.FloatingMenu.addButton(scope, that.buttons[button].button, GENTICS.Aloha.i18n(GENTICS.Aloha,
							"floatingmenu.tab.format"), 1);
					break;
				case "p":
				case "h1":
				case "h2":
	...
	this.multiSplitItems2 = new Array();
	jQuery.each(GENTICS.Aloha.Format.sizes, function(index, value) {
		var button = 'butt' + value;
		that.multiSplitItems2.push({
			name : button,
			tooltip : '', //that.i18n("button." + button + ".tooltip"),
			iconClass : '', //"GENTICS_button " + that.i18n("GENTICS_button_" + button),
			markup : jQuery('<b></b>'), //jQuery("<" + button + "></" + button + ">"),
			click : function() { alert('jojo'); }
		});
	});
	if (this.multiSplitItems2.length > 0) {
		this.multiSplitButton2 = new GENTICS.Aloha.ui.MultiSplitButton({
			items : this.multiSplitItems2
		});
		GENTICS.Aloha.FloatingMenu.addButton(scope, this.multiSplitButton2, GENTICS.Aloha.i18n(GENTICS.Aloha, "floatingmenu.tab.format"), 3)
	}
